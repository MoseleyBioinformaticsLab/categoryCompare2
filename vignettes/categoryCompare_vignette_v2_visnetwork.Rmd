...
% \VignetteDepends{categoryCompare2, GO.db, affy, hgu95av2.db, genefilter, estrogen, limma, DiagrammeR, visNetwork}
...
We can use the `DiagrammeR` and `visNetwork` html widgets to create interactive
visualizations either in the `RStudio` viewer, or as panes in the html report.
...
bp_comm_labels <- label_go_communities(bp_communities)
```

### Create Stats Table

To provide a list of which GO terms are in each of the communities we found,
lets generate a table, with the community labels so it makes it easier to find 
them in the graph if desired.
...
```{r bp_legend, echo = FALSE, results='asis'}
generate_legend(bp_assign, img = TRUE)
```

```{r}
annotation_table <- annotation_gene_table(bp_combined, graph::nodes(bp_graph))
```
...
```{r out_example, eval = FALSE}
kable_annotation_table(annotation_table, header = 4)
csv_annotation_table(annotation_table, out_file = "bp_annotations.csv")
```


```{r out_table1, results='asis', echo=FALSE}
kable_annotation_table(annotation_table[1:3], header_level = 4)
```
