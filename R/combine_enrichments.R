...
#' @export
#' @rdname combine_enrichments
setMethod("combine_enrichments", signature = "list", function(...) {
  .combine_enrichments_single(...)
})
...
.combine_enrichments <- function(enriched) {
  ...
  all_annotation <- combine_annotations(lapply(enriched, function(x) {
    x@annotation
  }))

  # create a dummy that may be a little smaller for passing to other functions
  out_combined <- new("combined_enrichment", enriched = enriched)
...
setMethod(
  ...
  function(comb_enrichment, annotation_similarity, low_cut, hi_cut) {
    .generate_annotation_graph(
      comb_enrichment,
...
.generate_annotation_graph <- function(
  comb_enrichment,
  ...
  keep_annotations <- names(annotation_features)[
    (n_features >= low_cut) & (n_features <= hi_cut)
  ]
  ...
  annotation_graph <- generate_annotation_similarity_graph(
    annotation_features[in_graph_annotation],
    annotation_similarity
  )

  annotation_graph <- add_data_to_graph(annotation_graph, annotation_table)

  annotation_graph@significant <- comb_enrichment@statistics@significant@significant[
    in_graph_annotation,
    ,
    drop = FALSE
  ]
...
add_data_to_graph <- function(graph, data) {
  ...
  for (i_data in names(data_types)) {
    ...
    attr(graph::nodeDataDefaults(graph, i_data), "class") <- type_convert[
      use_type
    ]
...
.generate_table <- function(comb_enrichment, link_type = "explicit") {
  ...
  if (has_desc) {
    if ((link_type != "explicit") & (has_link)) {
      text_desc <- paste(
        '<a href="',
        annot_obj@links[keep_annot],
        '">',
        annot_obj@desc[keep_annot],
        '</a>',
        sep = ""
      )
...
setMethod("combine_annotations", signature = "list", function(annotation_list) {
  .combine_annotations(annotation_list)
})

.combine_annotations <- function(annotation_list) {
  if (length(annotation_list) > 1) {
    combined_type <- unique(unlist(lapply(annotation_list, function(x) {
      x@annotation_type
    })))

    combined_feature_type <- unique(unlist(lapply(annotation_list, function(x) {
      x@feature_type
    })))
    ...
    if (n_type != 1) {
      stop(
        "Cannot combine annotation's with more than one annotation type.",
        call. = FALSE
      )
    }

    combined_features <- combine_annotation_features(lapply(
      annotation_list,
      function(x) {
        x@annotation_features
      }
    ))

    all_annotation_names <- names(combined_features)

    combined_description <- combine_text(
      lapply(annotation_list, function(x) {
        x@description
      }),
      all_annotation_names,
      "description"
    )
    combined_links <- combine_text(
      lapply(annotation_list, function(x) {
        x@links
      }),
      all_annotation_names,
...
combine_annotation_features <- function(annotation_features) {
  annotation_names <- lapply(annotation_features, function(x) {
    names(x)
  })
  ...
  for (i_annot in seq(1, length(annotation_features))) {
    ...
    annotation_out[use_names] <- lapply(use_names, function(x) {
      union(annotation_out[[x]], annotation_features[[i_annot]][[x]])
    })
  }
...
combine_text <- function(list_characters, names_out, text_id) {
  ...
  if (n_multi != 0) {
    errmsg <- paste(
      n_multi,
      text_id,
      "are not identical across annotation objects",
      collapse = " "
    )
...
#' @export
#' @return cc_graph
generate_annotation_similarity_graph <- function(
  annotation_features,
  ...
  all_comparisons <- all_comparisons[
    (all_comparisons[, 2] > all_comparisons[, 1]),
  ]
  ...
  calc_similarity <- function(x) {
    ...
    similarity_value <- 0
    if (len_intersect != 0) {
      similarity_value <- switch(
        similarity_type,
        overlap = len_intersect / min_len,
        jaccard = len_intersect / len_union,
        combined = (len_intersect * (min_len + len_union)) /
          (min_len * len_union) /
          2
      )
    }
  ...
  if (have_parallel()) {
    similarity <- parallel::mclapply(
      seq(1, nrow(all_comparisons)),
      calc_similarity
    )
  ...
  attr(
    graph::edgeDataDefaults(out_graph, attr = "weight"),
    "class"
  ) <- "FLOATING"
...
.extract_statistics_combined_enrichment <- function(in_results) {
  all_data <- lapply(in_results@enriched, function(x) {
    extract_statistics(x@statistics)
  })
