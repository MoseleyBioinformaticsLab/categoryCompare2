...
check_package_installed = function(packagename) {
  ...
  if (inherits(is_install, "try-error")) {
    stop_message = paste0(
      packagename,
      " is not installed, but is required for this operation!"
    )
    stop(stop_message)
  } else {
    return(NULL)
...
.annotation_combinations <- function(sig_matrix) {
  ...
  if ((nrow(unique_combinations) > 1) && (ncol(unique_combinations) >= 1)) {
    ...
    uniq_order <- do.call(
      order,
      c(
        lapply(1:n_col, function(i) unique_combinations[, i]),
        decreasing = TRUE
      )
    )
  ...
  group_description <- vapply(
    rownames(unique_combinations),
    function(in_comb) {
      use_comb <- unique_combinations[in_comb, , drop = FALSE]
  ...
  for (in_comb in name_combinations) {
    tmp_comb = unique_combinations[in_comb, ]
    has_match = rep(FALSE, nrow(sig_matrix))

    for (irow in seq_len(nrow(sig_matrix))) {
      has_match[irow] = identical(sig_matrix[irow, ], tmp_comb)
    }
  ...
  new(
    "node_assign",
    groups = unique_combinations,
    assignments = combination_assign,
...
generate_piecharts <- function(grp_matrix, use_color) {
  ...
  tmp_matrix = as.data.frame(matrix(
    1,
    nrow = nrow(piechart_strings),
  ...
  tooltips <- vapply(
    ...
    function(in_node) {
      ...
      for (i_dat in node_data) {
        out_tooltip <- paste0(
          out_tooltip,
          graph::nodeData(in_graph, in_node, i_dat),
          "\n"
        )
      }
...
#' @export
#' @return something
vis_in_cytoscape <- function(
  in_graph,
  in_assign,
  description = "cc2 enrichment"
) {
  ...
  graph::nodeData(
    in_graph,
    names(in_assign@assignments),
    "visattr"
  ) <- in_assign@assignments

  cyt_window <- RCy3::createNetworkFromGraph(
    graph = in_graph,
    title = description
  )
  ...
  if (in_assign@color_type == "solid") {
    RCy3::setNodeColorMapping(
      "visattr",
      names(in_assign@colors),
      colors = in_assign@colors,
...
setMethod(
  "remove_edges",
  signature = list(edge_obj = "character", cutoff = "numeric"),
  function(edge_obj, cutoff, edge_attr, value_direction) {
    .remove_edges_cw(edge_obj, cutoff, edge_attr, value_direction)
  }
)

.remove_edges_cw <- function(
  edge_obj,
  cutoff,
  edge_attr = "weight",
  value_direction = "under"
) {
  edge_data <- RCy3::getTableColumns('edge', network = edge_obj)

  switch(
    value_direction,
    under = to_delete <- edge_data[
      (as.numeric(edge_data[, edge_attr]) < cutoff),
    ],
    over = to_delete <- edge_data[
      (as.numeric(edge_data[, edge_attr]) > cutoff),
    ]
  )
...
setMethod(
  "remove_edges",
  signature = list(edge_obj = "cc_graph", cutoff = "numeric"),
  function(edge_obj, cutoff, edge_attr, value_direction) {
    .remove_edges_ccgraph(edge_obj, cutoff, edge_attr, value_direction)
  }
)

.remove_edges_ccgraph <- function(
  in_graph,
  cutoff,
  edge_attr = "weight",
  value_direction = "under"
  ...
  if (length(del_edges) > 0) {
    ...
    from_node <- sapply(del_edges, function(x) {
      x[1]
    })
    to_node <- sapply(del_edges, function(x) {
      x[2]
    })
...
graph_to_visnetwork <- function(
  ...
  in_graph <- add_tooltip(
    in_graph,
    description = in_assign@description,
    separator = "<br>"
  )
  ...
  if (!is.null(node_communities)) {
    comm_nodes <- unique(unlist(lapply(node_communities, function(x) {
      x$members
    })))
  ...
  from_to <- dplyr::filter(
    from_to,
    (.data$from %in% use_nodes) & (.data$to %in% use_nodes)
  )

  from_to$edgeid <- paste0(from_to$from, "|", from_to$to)
  from_to$weight <- edge_weight[from_to$edgeid]

  g_nodes <- DiagrammeR::create_node_df(
    n = length(use_nodes),
    label = use_nodes,
  ...
  for (inode in g_nodes$label) {
    ...
    g_nodes[which_node, "tooltip"] <- g_nodes[which_node, "title"] <- gsub(
      "\\n",
      "<br>",
      graph::nodeData(in_graph, inode, "tooltip")[[1]]
    )
  }
  ...
  if (!is.null(node_communities)) {
    ...
    for (icomm in seq(1, length(node_communities))) {
      ...
      g_nodes[
        (g_nodes$label %in% comm_data$members),
        "community"
      ] <- comm_data$label
    }
  }
  ...
  g_edges <- DiagrammeR::create_edge_df(
    from = from_to$from_id,
    to = from_to$to_id,
...
label_communities <- function(community_defs, annotation) {
  ...
  get_rep_member <- lapply(community_defs, function(in_def) {
    def_counts <- member_annotation_counts[
      names(member_annotation_counts) %in% in_def
    ]
...
table_from_graph <- function(
  ...
  if (!is.null(in_assign)) {
    ...
    node_table_2 <- cbind(
      tmp_table_1,
      node_table[, seq(cut_table_loc + 1, ncol(node_table))]
    )
  } else {
  ...
  if (!is.null(community_info)) {
    in_members <- unique(unlist(lapply(community_info, function(x) {
      x$members
    })))
    ...
    community_info = purrr::map(
      seq(1, length(community_info)),
      function(comm_id) {
