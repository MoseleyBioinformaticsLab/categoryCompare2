...
gsea_feature_enrichment = function(
  gsea_features,
  min_features = 15,
  max_features = 500,
  return_type = "cc2",
  ...
  gsea_res = fgsea::fgsea(
    pathways = gsea_features@annotation@annotation_features,
...
#' convert gsea object
#'
#' Takes an `gsea_result`, and converts it to the table expected by `fgsea`.
#' This should only be done on those that have `gsea` as the *Enrichment Method*.
#'
#' @param in_gsea the gsea_result object
#'
#' @export
#' @return data.table
gsea_result_to_fgsea = function(in_gsea) {
  ...
  ranks = in_gsea@ranks
  results = data.table::as.data.table(in_gsea@statistics@statistic_data)
  rename_list = c(
    pval = "p",
    padj = "padjust",
    log2err = "log2err",
  ...
  results$pathway = in_gsea@statistics@annotation_id

  return(list(
    pathways = in_gsea@annotation@annotation_features,
    ranks = ranks,
    results = results
  ))
}
