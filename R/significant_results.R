...
#'
multi_query_list <- function(list_to_query, queries) {
  ...
  query_results <- purrr::map(queries, rlang::eval_tidy, list_to_query)
...
setMethod(
  ...
  function(in_results, ...) {
    .get_significant_stat_results(in_results, rlang::enquos(...))
  }
)
...
setMethod(
  ...
  function(in_results, ...) {
    .get_significant_combined_enrichment(in_results, rlang::enquos(...))
  }
)

.get_significant_combined_enrichment <- function(in_results, queries) {
  stopifnot(length(queries) > 0)

  all_measured <- lapply(in_results@enriched, function(x) {
    x@statistics@annotation_id
  })

  all_significant <- lapply(in_results@enriched, function(x) {
    get_significant_annotations(x@statistics, queries)
  })
